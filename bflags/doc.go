/*
			...
			The library also supports specifying additional tags:
	        - 'meta': followed by a comma separated list of values that are attached
	          as a slice of strings to the 'Annotations' field of the resulting FlagBond
				`meta:"value1,value2"`
	        - 'viper': for binding to viper
				`viper:"node.file"`
			...
			NOTES
				* inner structs - even anonymous - can be used for bindings BUT the
				  inner structs need to be initialized otherwise an error is raised
				  since no binding would occur for their fields.
*/
package bflags
