...
// ErrorSilencer may be implemented by a type bound to a command in BindRunE for
// requesting to silence errors.
type ErrorSilencer interface {
	// SilenceErrors returns true to set cobra.Command.SilenceErrors to true
	SilenceErrors() bool
}

// ErrorTraceRemover may be implemented by a type bound to a command in BindRunE for
// requesting that errors returned by the RunE function have their stack trace removed.
